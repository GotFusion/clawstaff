...
**验收标准**
- [x] 至少 3 个示例任务成功转换并可被 OpenClaw 读取（`make skills-sample` + `make skills-validate-sample`）。
...
- `Makefile`（`release-regression` / `release-preflight`）
...
### C. 数据治理
- [ ] 明确保留周期和清理机制。
- [x] 支持按 session/task 导出。
  - `learning bundle` 导出脚本与 gateway 已支持 `--session-id` / `--task-id` 过滤，并可自动补齐依赖闭环。
...
- [x] 三模式均有可演示最小链路。
- [x] 从点击采集到 skill 执行形成闭环。
- [x] 执行日志可审阅、可追踪、可回放。
- [x] 所有关键行为有文档、有测试、有回退策略。
