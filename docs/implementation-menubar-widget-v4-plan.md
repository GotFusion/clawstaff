...
### Phase B 完成记录（2026-03-09）
- 已将 compact 主体从球体+胶囊改为低透明圆角方框（`fill 18% / stroke 38%`），并保留轻阴影。
- 已移除高饱和渐变球体与强调图标，精简模式仅展示“当前任务 / 下一步 / 点击展开(或收起)”三类信息。
- 精简模式整块区域保持 `Button` 点击热区，点击任意位置均可切换详细/精简。
- 前台部件初始窗口尺寸同步调整为方框版默认尺寸（`392 x 178`）。
...
### Phase C 完成记录（2026-03-09）
- 详细模式移除重背景卡片，仅保留轨道、节点与文本层，窗口底板降为极低透明度。
- 一级/二级节点统一透明色调：一级 `fill 45% / stroke 60%`，二级 `fill 32% / stroke 46%`。
- 时间轴布局按规范收敛：一级节点最小高度 `184`、一级到二级 `34`、二级条目间距 `46`、组间附加间距 `38`、轨道安全边距 `64`、文本起始偏移 `28`。
- 详细模式增加紧急停止细红线提示（含轻量状态文案），不使用大面积红色警示块。
...
### Phase D 完成记录（2026-03-09）
- 菜单栏从 `MenuBarExtra.window` 收敛为 `MenuBarExtra.menu`，采用系统原生菜单样式。
- 菜单项层级改为系统分组（控制台/部件显隐/部件模式子菜单/刷新/安全操作/退出），移除自定义容器与额外面板视觉。
- 危险操作仅保留文字红色强调（“触发紧急停止”“退出程序”），其余项保持系统默认样式。
- 菜单栏切换“精简/详细”模式统一走 `setModeFromMenu`，会自动确保前台部件显示，满足隐藏态联动要求。
