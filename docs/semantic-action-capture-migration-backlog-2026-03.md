...
更新时间：2026-03-26  
状态：待执行（Approved for implementation）
...
- 任务：
  - 编写 ADR：语义优先、坐标禁用、失败即停策略。
  - 在执行入口加入 `semantic_only=true` 强制开关。
  - 为旧入口增加显式错误提示（拒绝坐标执行）。
- DoD：
  - 合并后的代码中不存在可直达的坐标执行路径。
  - 单测覆盖：坐标执行请求返回明确错误码和原因。
  - ADR 审核通过并落地到 docs/adr。
- 风险：
  - 历史流程临时不可用。
