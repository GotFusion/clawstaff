...
- 完成 `SEM-201` Semantic Executor v1（2026-03-27）：新增 `SemanticActionStore.swift` 与 `SemanticActionExecutor.swift`，让 `OpenStaffReplayVerifyCLI` 可直接消费 `semantic_actions` SQLite 中的 action 记录，执行 `switch_app / focus_window / click / type / shortcut / drag` 的语义解析与 dry-run/live actuation，并把 `selector_hit_path_json / duration_ms / status / error_code` 统一回写到 `action_execution_logs`。执行链路默认拒绝 `coordinateFallback` 作为决策输入；当前 live drag 先支持 `window_move`，其余 drag intent 以结构化 blocked 收口。新增 Swift 单测与 Python CLI 集成回归已覆盖 click/shortcut/drag dry-run、坐标回退拦截，以及 `SQLite -> CLI -> action_execution_logs` 闭环。
- 完成 `SEM-202` Context Guard（2026-03-27）：`SemanticActionExecutor` 已在真正 resolve / actuation 前新增统一 preflight，上下文会按 `requiredFrontmostApp / windowTitlePattern / urlHost` 三层严格校验；一旦 mismatch，即返回 `SEM202-CONTEXT-MISMATCH` 并附带结构化 `contextGuard.requirements / actual / mismatches`，CLI 同步把这份原因写入 `action_execution_logs.result_json`。`ReplayEnvironmentSnapshot` 也已扩展 `url / urlHost`，浏览器场景支持优先读取 `AXDocument / AXURL` 做 host 级校验；同时保留 `contextGuard.disabledDimensions` 作为后续局部放宽入口。新增 Swift 单测与 Python 集成回归已覆盖错误 app、错误窗口标题和浏览器 host 不匹配三类前置拦截。
- 完成 `SEM-203` Post-Assertion Engine（2026-03-28）：`SemanticActionExecutor` 已在 live actuation 成功后自动抓取执行后 snapshot，并按 `requiredFrontmostApp / windowTitlePattern / selectorResolvable / targetSelectorResolvable / textValueContains` 评估结构化 `postAssertions`。其中 `requiredFrontmostApp / windowTitlePattern / textValueContains` 默认作为必需断言，失败会把整次动作升级为 `SEM203-ASSERTION-FAILED`；`selectorResolvable / targetSelectorResolvable` 先作为可选诊断断言，避免高波动 UI 因目标消失被误报。`ReplayElementSnapshot` 现已补充 `valueText`（仅非敏感 AX value）用于 `type` 后验证，CLI 会把 `postAssertions` 同步写入 `action_execution_logs.result_json`。新增 Swift 回归已覆盖 switch_app 后置失败拦截、type 值变化通过，以及原有 dry-run/context guard 路径不回归。
- 完成 `SEM-301` 历史任务转换器（2026-03-28）：`migrate_semantic_actions.py` 现已支持识别“仅剩坐标 fallback”的历史 GUI turn，自动回读 `observationRef.rawEventLogPath + taskChunkPath`，用 `semantic_action_builder` 重新构建动作窗口并按 `sourceEventIds` 匹配恢复语义 selector / args / assertions。对无法可靠恢复的步骤，迁移结果会强制写 `manual_review_required=true`，并在 `context_json.historicalConversion.reasonCode` 中记录诸如 `SEM301-MISSING-RAW-EVENT-LOG / SEM301-NO-HISTORICAL-MATCH / SEM301-RECOVERED-REQUIRES-MANUAL-REVIEW` 等明确原因；backfill 摘要新增 `historicalCoordinateCandidateCount / historicalAutoConvertedCount / historicalAutoConversionRate / historicalConversionReasonCounts`，新增 `tests/integration/test_historical_coordinate_converter.py` 已固定验证 `4` 条历史样本里 `3` 条自动转换成功，自动转换率 `75%`。
- 完成 `SEM-302` 审核工作流（Teacher Confirmation，2026-03-28）：`SemanticActionExecutor` 已在 context guard 之后、真正执行之前新增 `teacher confirmation guard`，默认对 `manual_review_required`、低于阈值的 selector、`switch_app`、`drag` 与批量 `type` 动作触发审核，未传 `--teacher-confirmed` 时统一以 `SEM302-TEACHER-CONFIRMATION-REQUIRED` 阻断。`OpenStaffReplayVerifyCLI` 现支持 `--teacher-confirmed`、`--teacher-confirmation-root` 与 `--teacher-confirmation-policy`，会把候选 selector、上下文、断言、原因码与老师决定同时写进 `action_execution_logs.result_json.teacherConfirmation`，并额外落独立 artifact 到 `teacher-confirmations/{date}/{sessionId}/`，供后续学习链路引用。新增 `config/semantic-teacher-confirmation.example.json`、Swift 单测与 Python CLI 集成回归，已覆盖低置信 click 阻断、drag 审核放行、阈值配置下调和 artifact 回写。
- 完成 `SEM-303` 可观测性与指标看板（2026-03-28）：新增 `scripts/observability/build_semantic_action_dashboard.py`、`scripts/observability/README.md`、`config/semantic-action-observability.v0.json` 与 `docs/metrics/semantic-action-observability-metrics.md`，可直接从 `semantic_actions` SQLite 聚合 `selectorHitRate / fallbackLayerDistribution / interceptRate / replaySuccessRate / manualConfirmationRate / misTriggerRiskEventCount`，并同时产出 `metrics-summary.json + dashboard.md`。`OpenStaffReplayVerifyCLI` 现会在 `action_execution_logs.result_json` 固定写入 `environment`，看板可按 `dev / staging / prod` 分维度展示；一旦出现 `SEM202-CONTEXT-MISMATCH / SEM203-ASSERTION-FAILED / SEM201-COORDINATE-FALLBACK-DISALLOWED`，摘要会自动生成风险告警并可通过 `--check-gates` 直接 fail。新增 `tests/integration/test_semantic_action_observability_dashboard.py` 已覆盖环境分桶、指标聚合、Markdown 看板与 gate fail。
- 完成 `SEM-401` 端到端基准集与回归套件（2026-03-28）：新增 `data/benchmarks/semantic-action-e2e/catalog.json` 与 `docs/semantic-action-e2e-benchmark-spec.md`，冻结 `8` 条 committed snapshot case，覆盖 `switch_app / focus_window / click / type / shortcut / drag / multi_display / browser_url` 八类高风险语义动作路径；新增 `scripts/benchmarks/run_semantic_action_e2e_benchmark.py`，可自动物化 `semantic_actions` SQLite、驱动 `OpenStaffReplayVerifyCLI`、输出 `source-record / case-report / attempt-report / cli-report / execution-log`，并支持 `--max-retries` 做 flake 重跑。`scripts/release/run_regression.py`、`Makefile` 与 `.github/workflows/semantic-action-e2e-benchmark.yml` 也已接通，让这套 benchmark 在 PR / `main` / nightly 上持续执行；新增 `tests/integration/test_semantic_action_e2e_benchmark.py` 与 `tests/integration/test_release_regression.py` 已固定验证 case 覆盖、产物结构和发布门禁接线。
- 完成 `SEM-402` 性能与鲁棒性优化（2026-03-28）：`SemanticTargetResolver` 现会对 snapshot candidate 建 `axPath / identifier / role` 索引，并优先在近邻候选里做 `roleAndTitle / textAnchor` 匹配，减少 selector resolve 扫描成本；`SemanticActionExecutor` 对 dry-run 复用初始 snapshot，对 post-assertion 复用 resolution cache，并在 live actuation 后按 `postAssertionSnapshotMaxAttempts` 做 snapshot retry，吸收短暂 UI 不稳定。`semantic_action_builder.py` 同时新增高频重复 click 降噪，避免极短时间抖动事件被误建成多个 click。为此新增 `scripts/benchmarks/aggregate_semantic_action_e2e_metrics.py`、`data/benchmarks/semantic-action-e2e/metrics-v0.json` 与 `tests/integration/test_semantic_action_e2e_metrics.py`，把 `p95ActionDurationMs / maxActionDurationMs / timeoutCount / stabilityPassRate / postActionSnapshotRecoveryRate` 收敛进 `SEM-402` gate；release regression、`benchmark-semantic-e2e-preflight` 与 GitHub Actions 现默认以 `repeatCount=3` 跑压力回归。
- 完成 `SEM-501` 正式切流（工程侧，2026-03-28）：`OpenStaffOpenClawCLI` gateway 不再要求调用方透传 `--semantic-only`，而是固定按 semantic-only 执行；`OpenClawRunner` 也移除了对该 runtime flag 的依赖，仅保留 `semanticOnly=true` 的请求级硬约束。新增 `docs/semantic-only-cutover-runbook.md`，把 staging -> prod 切流、观测门槛与“只回滚版本、不恢复坐标执行”的预案固定成发布流程；同时补齐 Swift / Python 回归，确保 coordinate-only skill 即使直打 gateway 也只会被 semantic preflight 拦截，不会重新进入坐标执行路径。
- 完成 `SEM-502` 技术债清理与文档收口（2026-03-28）：已从 `apps/macos` 包内物理移除 `OpenStaffActionExecutor`、`OpenStaffExecutorXPCClient`、`OpenStaffExecutorHelper` 与对应 shared target，`LearnedSkillRunner` 现统一通过新的 `LearnedSkillOpenClawBridge` 调 `OpenStaffOpenClawCLI`，不再保留任何 App 内坐标执行桥。`SEM-003` guard 也已升级为零 allowlist，并同步更新 `user-manual`、release/validation 文档与 `docs/semantic-action-migration-retrospective-2026-03.md` 迁移复盘。
...
- 完成阶段 11.0.1 Learning Status Surface：新增 `LearningStatusSurfaceCard`、学习状态轮询与暂停/恢复链路，首页与前台部件现可统一展示 `learning on / paused / excluded / sensitive-muted`、最近落盘时间、当前 app 与一键暂停入口；`LearningStatusSurfaceTests` 已固定回归状态文案与交互。
...
- 完成阶段 11.1.4 `InteractionTurnBuilder` 诊断补强：`InteractionTurn` 新增可选 `buildDiagnostics`，`InteractionTurnBuilder.buildResult` 会对缺失的 `sourceRecord/rawEvent/taskChunk/semanticTarget/execution/review` 链路产出结构化 diagnostics；`scripts/learning/build_interaction_turns.py` 也已同步写出 `buildDiagnostics`、execution log / repair request / benchmark linkage，并在回填摘要中汇总诊断计数。
...
- 完成阶段 11.2.4 信号合并器：新增 `core/learning/PreferenceSignalMerger.swift`，按“同一 turn + scope + type + polarity”合并多来源 signal，自动去重、合并 evidence / confidence，并为 `opposing_polarity`、`divergent_hint`、`divergent_proposed_action` 等冲突打标签；`DirectiveHintBuilder` 与 `PreferenceRulePromoter` 已接入该合并结果，避免同一 turn 的规则信号被重复计数或生成重复 hint。
...
- 完成阶段 11.6.4 / 11.6.5 learning bundle 导出、校验与恢复：新增 `scripts/learning/learning_bundle_common.py`、`scripts/learning/export_learning_bundle.py`、`scripts/learning/verify_learning_bundle.py`、`docs/learning-bundle-spec.md` 与 `docs/adr/ADR-0022-learning-bundle-export-and-restore.md`，把 `turns / evidence / signals / rules / profiles / audit` 收敛为可迁移 bundle，并固定输出 `manifest.json + verification.json`；restore 现在默认先做 dry-run 预览，只有显式 `--apply` 才会把 payload 恢复到标准 workspace 布局，同时自动恢复 `profiles/latest.json`。新增 Python integration 回归已覆盖 `export -> verify -> restore -> preference-profile rebuild` 全链路，并验证恢复后新 profile 仍对齐原有 `ruleId`。
- 完成阶段 11.6.6 hook / gateway 集成边界：新增 `core/contracts/LearningIntegrationContracts.swift`、`core/storage/LearningGateway.swift` 与 `docs/integrations/learning-hooks-gateway-v0.md`，把 `learning.turn.created / learning.signal.extracted / preference.rule.promoted / preference.profile.updated` 四类 hook 事件，以及 `preferences.listRules / preferences.listAssemblyDecisions / preferences.exportBundle` 三个 gateway 方法收敛到统一公开边界；`FileSystemLearningGateway` 现已复用既有 rule store、assembly store 与 learning bundle 导出脚本，并通过 `LearningGatewayTests` 固定验证外部只依赖公开 request/response 也能消费学习结果。
