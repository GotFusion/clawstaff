...
- runner 调 gateway 时必须透传 `--semantic-only`。
- direct gateway / legacy entry 若缺少 `--semantic-only`，返回显式错误：
  - `OCW-SEMANTIC-ONLY-REQUIRED`
...
- direct gateway 的调用协议更严格，需要调用方补上 `--semantic-only` 与透传参数。
